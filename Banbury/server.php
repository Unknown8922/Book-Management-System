...
function proceedNewRequestBook($bookid, $customerid, $bookQuantity, $cardNumber, $address)
{
	...
	else
	{
       ...
       $sqlQuery =   "INSERT INTO `banbury_db`.`order` (Oid, `bid`, `customer_id`, `card_number`, `delivery_address`, `order_Date`) VALUES (NULL, '$bookid', '$customerid', '$cardNumber', '$address', '$todayDate');";
...
$sql ="INSERT INTO `banbury_db`.`order_details` (`Oitem_ID`, `OID`, `BID`, `Qty`) VALUES
...
*/
  ...
  $query = "SELECT title, price, author FROM `banbury_db`.`books` WHERE "; 
    //$and = "0"; 
...
$server = new SoapServer("http://localhost/Dubai_Books/Banbury/service.wsdl");
