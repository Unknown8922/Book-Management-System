...
function proceedNewRequestBook($bookid, $customerid, $bookQuantity, $cardNumber, $address)
{
	...
	$conn = new mysqli("localhost", "root", "", "cerebro_db");
	...
	else
	{
       ...
       $sqlQuery =   "INSERT INTO `cerebro_db`.`order` (Oid, `bid`, `customer_id`, `card_number`, `delivery_address`, `order_Date`) VALUES (NULL, '$bookid', '$customerid', '$cardNumber', '$address', '$todayDate');";
...
$sql ="INSERT INTO `cerebro_db`.`order_details` (`Oitem_ID`, `OID`, `BID`, `Qty`) VALUES
...
*/
  ...
  $query = "SELECT title, price, author FROM `cerebro_db`.`books` WHERE "; 
    //$and = "0"; 
...
$server = new SoapServer("http://localhost/Dubai_Books/Cerebro/service.wsdl");
